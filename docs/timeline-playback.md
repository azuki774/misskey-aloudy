...
2. Click 読み上げ ON → the page creates a `PlaybackPipeline`, a `PlaybackState`, and a `VoiceVoxPlayer`. New notes that arrive from the global timeline are auto-enqueued and read aloud via VoiceVox.
...
| Reading status labels | `OFF` / `読み上げ準備中…` / `読み上げ中` / `一時停止中` / `エラー: <msg>` | Distinct from the connection-state labels so the two are not confused. |
| Currently-playing badge | A small inline element inside the `<li>`, hidden by default, toggled via `markNotePlaying` / `unmarkNotePlaying` driven by `pipeline.on("noteStart" / "noteEnd")` | Cheap, no CSS changes needed. We use `data-note-id` on each `<li>` to find the matching one. |
...
| Queue size indicator | "再生中 (残り N 件)" in the reading status, updated via `pipeline.on("queueChange")` | Small UX win. Lets the user know "there's a backlog". |
| "再生中" badge on `<li>` | Inner `<span class="playing-badge">`, hidden by default, shown via `removeAttribute("hidden")` | Same pattern as the existing error region. |
...
└── scripts/index.ts       # modified — wire up the pipeline
docs/
└── timeline-playback.md   # new — this file
```

No library changes. The pipeline (#19), state (#16), queue (#17) are already complete and untouched.
...
3. User clicks 読み上げ ON.
   - New PlaybackState, VoiceVoxPlayer, PlaybackPipeline are created
     and wired together.
   - pipeline.on("noteStart") / "noteEnd" update the "再生中" badge
     on the matching <li> via data-note-id.
   - state.on("statechange") updates the reading status display.
   - pipeline.on("queueChange") updates the "残り N 件" counter.
...
4. New note arrives.
   ...
   - The pipeline dequeues, converts to text via toReadingText (#14),
     synthesizes via the voicevox client (#8), plays via the player
     (#9), then dequeues the next one.
5. User clicks 読み上げ OFF.
   - pipeline.stop() pauses the in-flight audio. Queue preserved.
   - State becomes "paused". UI updates.
...
function enableReading(): void {
  ...
  pipeline = new PlaybackPipeline({ client, player, state: readingState });
  ...
  readingState.on("statechange", ({ to }) => setReadingStatus(READING_LABELS[to]));
  pipeline.start();
  updateReadingButtons();
  setReadingStatus(READING_LABELS[readingState.state]);
}
...
function disableReading(): void {
  ...
  isReading = false;
  pipeline.stop();
  // unmark the current "再生中" badge, if any
  if (readingState?.currentNote) unmarkNotePlaying(readingState.currentNote.id);
  updateReadingButtons();
  setReadingStatus("OFF");
}
...
function handleNote(note: Note): void {
  addNote(note);                    // existing display logic
  if (isReading && pipeline) {
    pipeline.enqueue(note);         // TTS path
  }
}

// onDisconnectClick: destroyPipeline() FIRST, then existing logic
// beforeunload: also call destroyPipeline()
```

## 7. Edge cases (handled)

- Reading toggled ON while not connected → button is disabled, so this can't happen via UI.
- Reading toggled OFF while a note is being played → `pipeline.stop()` calls `player.stop()` which terminates the audio immediately. The pipeline state goes to `paused`. The "再生中" badge is removed.
...
- Pipeline is destroyed mid-`synthesize` → the `synthesize` promise may complete, but the loop's `destroyed` check bails out before playing the audio. No leak.
...
No automated tests added for the browser script (would require a DOM fixture). The unit tests for the underlying libraries (`MisskeyClient`, `subscribeGlobalTimeline`, `PlaybackState`, `NoteQueue`, `PlaybackPipeline`) cover the logic.
...
- [ ] Click 読み上げ ON. The reading status becomes "読み上げ中" (or briefly "読み上げ準備中…").
- [ ] Wait for a note to arrive. The note should have a "再生中" badge briefly while it's being read.
- [ ] After reading, the badge is removed and the next note (if any) is read.
- [ ] Click 読み上げ OFF. The in-flight reading stops. The reading status becomes "OFF". Re-click ON to resume (the next incoming note will be read).
